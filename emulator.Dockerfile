...
RUN cd /usr/src/ \
    && git clone --depth 1 https://github.com/arjpar/aristotle.git \
    && git config --global --add safe.directory /usr/src/aristotle

COPY --from=mupdf-libs /mupdf-${MUPDF_VERSION}-source.tar.gz /usr/src/aristotle/thirdparty/
RUN cd /usr/src/aristotle/thirdparty/ \
    && mkdir -p mupdf \
    && tar -xz --strip-components 1 --directory mupdf \
    --file mupdf-${MUPDF_VERSION}-source.tar.gz \
    && rm --verbose mupdf-${MUPDF_VERSION}-source.tar.gz

RUN cd /usr/src/aristotle/mupdf_wrapper \
    && ./build.sh \
...
COPY --from=plato-emulator-libs /usr/src/aristotle/target/ /usr/src/aristotle/target/
COPY --from=plato-emulator-libs /usr/src/aristotle/thirdparty/mupdf/ /usr/src/aristotle/thirdparty/mupdf/
